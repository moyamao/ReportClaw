    ...
    def extract_future_from_fulltext_by_ordinals(
        ...
        *,
        ...
        def _find_heading_pos_for_ordinal(text: str, ordinal: int) -> int | None:
            ...
            # We anchor to line starts (or after newline) to reduce false positives.
            patterns = [
                # 第十一部分 / 第11部分
            ...
            for pat in patterns:
                for m in re.finditer(pat, text, flags=re.IGNORECASE):
                    ...
                    # Reject obvious boilerplate / preface / TOC-ish hint lines
                    compact = re.sub(r"\s+", "", line)
                    if re.search(r"(章节|章节的相关内容|敬请|详见|请参阅|重要提示|目录|释义|声明|报告全文)", compact):
                        continue
